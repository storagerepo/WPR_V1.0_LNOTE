...
(function (window, $, undefined) {
	...
	$.html5imageupload = function html5imageupload(options, element) {
		...
		//buttons
		this.button				= {}
		this.button.edit		= '<div class="file btn-info btn-edit"><i class="icon-pencil"></i></div>'
		this.button.saving		= '<div class="file btn-warning saving">' + (this.options.saveLabel || 'Saving...') + ' <i class=" icon-spinner"></i></div>';
		this.button.zoomin		= '<div class="file btn-default btn-zoom-in"><i class=" icon-resize-full"></i></div>';
		this.button.zoomout		= '<div class="file btn-default btn-zoom-out"><i class=" icon-resize-small"></i></div>';
		this.button.zoomreset	= '<div class="file btn-default btn-zoom-reset"><i class=" icon-resize-full-3"></i></div>';
		this.button.cancel		= '<div class="file btn-danger btn-cancel"><i class=" icon-trash"></i></div>';
		this.button.done		= '<div class="file btn-success btn-ok"><i class=" icon-ok"></i></div>';
		this.button.del			= '<div class="file btn-danger btn-del"><strong><small>&nbsp;&nbsp;Change&nbsp;&nbsp;</small></strong></div>';
	...
	$.html5imageupload.prototype = {
		_init: function() {
			...
			if (!empty(options.image) && options.editstart == false) {
				...
				//var img			= $('<img class="preview"></img>').attr('src',options.image);
				//console.log(img);
				//$(element).append(img);
		...
		readImage: function(image, src, name, mimeType) {
    		...
    		img.onload 		= function(tmp) {
    			...
    			_self.image			= $(imgElement).clone().data({ mime: mimeType, width: width, height: height, ratio: ratio, left: left, top: top, useWidth: useWidth, useHeight: useHeight}).addClass('main').mousedown(function(event) { _self.imageHandle(event)});
